...
func (r *Repository) init() {
	r.db.Exec(`CREATE TABLE IF NOT EXISTS "todos" ("id" varchar(36) primary key,"message" varchar(255),"complete" bool )`)
}
...
func (r *Repository) ResolveAll() ([]Todo, error) {
	...
	if err != nil {
		log.Fatal("could not resolve all:", err)
		return todos, err
	}
	defer rows.Close()
...
func (r *Repository) ResolveByID(id uuid.UUID) (Todo, error) {
	var t Todo
...
func (r *Repository) Store(t Todo) error {
	stmt, err := r.db.Prepare("INSERT INTO todos (id, message, complete) VALUES (?, ?, ?)")
	if err != nil {
		log.Printf("failed to create statement: %v", err)
		return err
	}
	defer stmt.Close()
	_, err = stmt.Exec(t.ID, t.Message, t.Complete)
	if err != nil {
...
//Update todo data
func (r *Repository) Update(id uuid.UUID, t Todo) error {
